...
def calc_quantity(df, row_number, product_code, total_trade_dict):
    imp_exp_dict = {}
    for i in range(row_number):
        ...
        if df.iloc[i, 1] == 2020 and df.iloc[i, 12] == "World" and df.iloc[i, 21] == product_code:
            # create dictionary of import & export values per country {'country': {M: imp_q, X: exp_q, ...]}
            ...
            if "m" in df.iloc[i, 7].lower() and "r" not in df.iloc[i, 7].lower():
                imp_exp_dict[df.iloc[i, 9]]['M'] += [i, 25]
            elif "x" in df.iloc[i, 7].lower() and "r" not in df.iloc[i, 7].lower():
                imp_exp_dict[df.iloc[i, 9]]['X'] += [i, 25]
...
def price_bar(cp_dict, product):
    ...
    plt.title("Export/import price difference by country for " + product)

    plt.xticks(x_pos, countries)

    plt.savefig(product + '_bar_graph.png', dpi=400)
    plt.show()
    return
